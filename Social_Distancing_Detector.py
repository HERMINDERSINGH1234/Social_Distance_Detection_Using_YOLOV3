...
vs =cv2.VideoCapture(r"sample-video.mp4"if "people1.jpg"else 0)#r"pedestrian.mp4"if "pedestrian.mp4"else
writer =None
   ...
   # loop over the upper triangular of the distance matrix
        ...
        for i in range(0,D.shape[0]):
            for j in range(i+1,D.shape[1]):
                if D[i,j]<config.MIN_DISTANCE: # Min-Distance
                    violate.add(i)
                    violate.add(j)
    ...
    for(i,(prob,bbox,centroid)) in enumerate(results):
        ...
        if i in violate: #Violate
            color = (0,0,255)
        cv2.rectangle(frame,(startX,startY),(endX,endY),color,2)
        cv2.circle(frame,(cX,cY),5,color,1)
    
    text ="Social Distancing Violations: {}".format(len(violate)) #Violate
